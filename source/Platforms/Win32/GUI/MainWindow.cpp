...
void MainWindow::addChats()
{
	...
	wchar_t buffer[100];
	swprintf(buffer, sizeof(buffer), L"%d", chats.size());
	MessageBox(MainWindow::dialog, buffer, L"Success", MB_OK);
	
	EnableWindow(GetDlgItem(dialog, IDC_MAIN_EXPORT_ALL), chats.size() > size_t(0));
...
void MainWindow::resizeChildWindows(int width, int height)
{
	...
	SetWindowPos(GetDlgItem(dialog, IDC_MAIN_EXPORT_TXT), NULL, chatsWidth + border * 2, height - border - buttonRowHeight, 150, buttonRowHeight, SWP_NOZORDER | SWP_SHOWWINDOW);
	SetWindowPos(GetDlgItem(dialog, IDC_MAIN_EXPORT_HTML), NULL, chatsWidth + border * 2 + 180, height - border - buttonRowHeight, 150, buttonRowHeight, SWP_NOZORDER | SWP_SHOWWINDOW);
	SetWindowPos(GetDlgItem(dialog, IDC_MAIN_EXPORT_ALL), NULL, chatsWidth + border * 2 + 360, height - border - buttonRowHeight, 130, buttonRowHeight, SWP_NOZORDER | SWP_SHOWWINDOW);
}
...
void MainWindow::exportChatAll(std::vector<WhatsappChat *> chats)
{
	std::string templateHtml = imageDecoder.loadString(MAKEINTRESOURCE(IDR_CHAT_EXPORT_HTML_ALL_TEMPLATE), RT_HTML);
	ChatExporterHtml exporter(templateHtml);
